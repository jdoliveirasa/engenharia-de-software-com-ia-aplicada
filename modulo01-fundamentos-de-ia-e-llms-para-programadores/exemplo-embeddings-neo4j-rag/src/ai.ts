...
export class AI {
    ...
    async retrieveVectorSarchResults(input: ChainState): Promise<ChainState> {
        this.params.debugLog("* Buscando o vector store no Neo4j...");
        const vectorResults = await 
        this.params.vectorStore.similaritySearchWithScore(
            input.question,
            this.params.topK
        );
        ...
        this.params.debugLog(`* Encontramos ${vectorResults.length} resultados relevantes (melhor score: ${topScore.toFixed(3)})`);
    ...
    async generateNPLResponse(input: ChainState): Promise<ChainState> {

        if (input.error) return input;

        this.params.debugLog("* Gerando resposta com IA (modelo de linguagem)...");
    ...
    async answerQuestion(question: string) {
        const chain = RunnableSequence.from([
            this.retrieveVectorSarchResults.bind(this),
            this.generateNPLResponse.bind(this),
        ])
