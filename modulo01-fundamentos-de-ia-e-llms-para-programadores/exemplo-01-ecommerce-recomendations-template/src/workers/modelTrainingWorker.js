...
//Why? Keeps all feature balanced so no one dominates traning
...
function encodeProduct(product, context) {
    ...
    // aplicar o peso na recomentdação 
    ...
    const category = oneHotWeighted(
        context.categoriesIndex[product.category] ?? 0,
        context.numCategories,
        WEIGHTS.category        
    )

    const color = oneHotWeighted(
        context.colorsIndex[product.color] ?? 0,
        context.numColors,
        WEIGHTS.color
    )
...
function encodeUser(user, context) {    
    ...
    return tf.concat1d(
        [
            ...
            tf.zeros([context.numCategories]), // categoria ignorada, pois sem histórico de compras
            tf.zeros([context.numColors]) // cores ignoradas, pois sem histórico de compras
        ]
...
function createTrainingData(context) {
    ...
    const labels = []    
    context.users
    .filter(u => u.purchases.length) //focar em usuários com histórico de compras (melhor para treinar)
    .forEach(user => {
...
async function configureNeuralNetAndTrain(traingData) {    
    ...
    model.add(
        tf.layers.dense({ 
            inputShape: [traingData.inputDimension], 
            units: 128, 
            activation: 'relu' 
        })
    );
    ...
    model.compile({
        optimizer: tf.train.adam(0.01), // Otimizador Adam (ajusta pesos de forma inteligente)
        loss: 'binaryCrossentropy', // Função de perda para classificação binária
        metrics: ['accuracy'] // Métrica para avaliar desempenho durante o treino
    });

    await model.fit(traingData.xs, traingData.ys, {
...
async function trainModel({ users }) {
    ...
    const traintingData = createTrainingData(context)
    _model = await configureNeuralNetAndTrain(traintingData) 
...
function recommend({ user }) {
    ...
    const recommendations = context.productVectors
        .map((item, index) => ({
            ...item.meta,
            name: item.name,
            score: scores[index] //previsão do mode para este produto
        }))
